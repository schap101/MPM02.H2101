...
We evaluate the accuracy of both models by testing them with the test data partition. The RMSE value will serve as the accuray of the models.
...
```
We can see that for the predictor month all dummy variables are crossing the zero line, indicating that this variable is not statistically significant. Therefore we could have dropped this predictor as well even though the p-values where low in the model examination.
...
```
- We get a RMSE of 90.4. Comparing to the lm.fit.3 with a RMSE 99.3 we have an improvement of roughly 9%.

- Below for the plots with the smoothing splines for each variable with and without the residuals.

## Summary

- It can be stated that using a GAM model with smoothing splines improves the model compared to a basic linear model. We have tried to add smoothing splines to the categorical variables but somehow we were not able to run this function. This could have improved the model even more.
```{r, echo=FALSE, out.height="50%", fig.align = 'center', warning=FALSE}
par(mfrow=c(1,3))
plot(gam_5_train,se=TRUE,col="blue")
```
...
```{r, echo=TRUE, results='hide'}
glm.binomial.5 <- glm((registered/cnt) ~ yr + mnth + holiday + weekday +
                        workingday, data=day, family="quasibinomial")
...
```{r, echo=FALSE}
glm.poisson.2 <- glm(cnt ~ season + yr + mnth + holiday + weekday + workingday +
...
```{r, echo=TRUE, results='hide'}
glm.poisson.3 <- glm(cnt ~ season + yr + mnth + holiday + weekday + workingday +
                       weathersit + temp, data=day, family=quasipoisson)
...
### Plot Prediction against Real Data
```{r, echo=FALSE, out.width="40%", fig.align = 'center'}
plot(control_11, pred_11, col='orange', cex = .3, pch=20, ylab = "predicted counts NN", xlab = "real counts")
abline(0,1)
```
...
print(paste("RMSE Linear Model: ",RMSE_Linear_Model <- sqrt(mean((control - exp(lm.fit.3.test))^2))))

#RMSE Gam Model (RMSE of 90.4)
print(paste("RMSE GAM Model: ",RMSE_gam_5 <- sqrt(mean(control - exp(gam_5_test))^2)))
