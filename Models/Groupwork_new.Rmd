...
To form a usecase here, the company Capital BikeShare would like to predict the amount of rentals at a given time to be able to better plan the work resources in the future. We as a data science startup were engaged to estimate the number of bikes based on the data.
...
We consider the response variables as an amount altough we are a bit unsure here. However the results in the next chapter shows that using the log function improves the linear regression model quite a bit.

In all 4 scatter plots above the linear model lines (green) as well as the gam model lines (red) are plotted. For both temperature variables the rentals seems to increase until the temperature reaches a certain value.At the beginning of the observations (low temp.) ans well as the end (high temp.) the linear model seems to overestimate the number of rentals.
...
p1 <- ggplot(data = df,
       mapping = aes(y = log(cnt),
                     x = factor(hr),fill= hr)) + ylab("Num. of Bike Rentals [log]") + xlab("Hour") +  labs(fill="Hour") +
geom_boxplot() 

p2 <- ggplot(data = df,
       mapping = aes(y = log(cnt),
                     x = factor(weekday),fill= weekday)) + ylab("Num. of Bike Rentals [log]") + xlab("Weekday") +  labs(fill="Day") +
geom_boxplot()

p3 <- ggplot(data = df,
       mapping = aes(y = log(cnt),
                     x = factor(workingday),fill= workingday)) + ylab("Num. of Bike Rentals [log]") + xlab("Workingday") +  labs(fill="Day") +
geom_boxplot() 

p4 <- ggplot(data = df,
       mapping = aes(y = log(cnt),
                     x = factor(holiday),fill= holiday)) + ylab("Num. of Bike Rentals [log]") + xlab("Holidays") +  labs(fill="Day") +
geom_boxplot() 
...
As already mentioned in section before, the data at the 27 and 28 August is not complete and wrong. Therefore, we will drop it for the SVM as well.
...
```{r, echo=FALSE}
# Fit a linear model without the predictor atempereature and accessing both
# prediction performances with the test data and comparison of the
# R-squared value.
...
# take inverse of natural log to obtain RMSE with actual unit
...
print(summary(lm.fit.3.test)$r.squared)
print(summary(lm.fit.4.test)$r.squared)
```
...
gam_0_test <- gam(formula(gam_0_train),data=test)
gam_1_test <- gam(formula(gam_1_train),data=test)
gam_2_test <- gam(formula(gam_2_train),data=test)
gam_3_test <- gam(formula(gam_3_train),data=test)
gam_4_test <- gam(formula(gam_4_train),data=test)

predict_gam_0 <- predict(gam_0_test)
predict_gam_1 <- predict(gam_1_test)
predict_gam_2 <- predict(gam_2_test)
predict_gam_3 <- predict(gam_3_test)
predict_gam_4 <- predict(gam_4_test)

# take inverse of natural log to calculate RMSE with actual unit (number of bike rentals)
RMSE_gam_0 <- sqrt(mean((exp(control_log) - exp(predict_gam_0))^2))
RMSE_gam_1 <- sqrt(mean((exp(control_log) - exp(predict_gam_1))^2))
RMSE_gam_2 <- sqrt(mean((exp(control_log) - exp(predict_gam_2))^2))
RMSE_gam_3 <- sqrt(mean((exp(control_log) - exp(predict_gam_3))^2))
RMSE_gam_4 <- sqrt(mean((exp(control_log) - exp(predict_gam_4))^2))
...
```{r, echo=FALSE}
gam_5_test <- gam(formula(gam_5_train),data=test)
predict_gam_5 <- predict(gam_5_test)
# take inverse of natural log to calculate RMSE with actual unit (number of bike rentals)
...
```{r, echo=FALSE}
day_0 <- read.csv("../Data/day.csv")
# head(day_0)
```

```{r, echo=FALSE}
hour_0 <- read.csv("../Data/hour.csv")
# head(hour_0)
```
...
```{r, echo=FALSE}
day$season <- factor(day$season, levels = c("1", "2", "3", "4"), ordered = FALSE)
...
hour$season <- factor(hour$season, levels = c("1", "2", "3", "4"), ordered = FALSE)
...
```{r, echo=FALSE, eval=FALSE}
# First we have a look at how the different variables are corresponding with
# number of bicycles, that are being used daily: 
pairs.daily <- pairs(cnt ~ season + yr + mnth + holiday + weekday + workingday +
                       weathersit + temp + atemp + hum + windspeed + casual + 
                       registered, data = day, upper.panel = panel.smooth)
```
...
# Let's have a look at the number of bicycles, that are being used hourly:
pairs.hour <- pairs(cnt ~ season + yr + mnth + hr+ holiday + weekday +
...
```{r, echo=FALSE}
pairs.hour.new <- pairs(cnt ~ season + mnth + hr+ weathersit + temp + atemp +
...
# Here we can see that spring (season 1) is the least popular season for bike
# usage. This seems a little contraintuitiv as one would assume it would be
# winter. So we have a closer look at the usage per month.

ggplot(day,aes(x=(season= as.character(season)), y=cnt, fill=season)) + 
  geom_boxplot()+labs(x="Seasons", y="Number of Bikes Used per Day")

```

```{r, echo=FALSE, eval=FALSE}

# It looks like late winter/early spring is the least popular time for bike
# rentals. Let's check, if it might have something to do with the weather.
ggplot(day,aes(x=(mnth= month.abb[mnth]), y=cnt)) + 
  geom_boxplot()+labs(x="Months", y="Number of Bikes Used per Day") +
...
ggplot(day,aes(x=(mnth= month.abb[mnth]), y=temp) + 
...
```{r, echo=FALSE}
glm.binomial.hour.1 <- glm(ilogit(cnt) ~ season + yr + mnth + hr+ holiday +
                             weekday + workingday + weathersit + temp + atemp +
                             hum + windspeed, data = hour,
                           family = "quasibinomial")
print(summary(glm.binomial.hour.1))
```
...
```{r, echo=FALSE, eval=FALSE}
glm.binomial.hour.2 <- glm(ilogit(cnt) ~ season + mnth + hr + weekday + workingday + weathersit + temp + hum + windspeed + weathersit:temp + weathersit:windspeed + weathersit:hum, data = hour, family = "quasibinomial")

summary(glm.binomial.hour.2)
```
It looks like the weather situation does not have a significant impact and neither does wind speed or the interaction of the weather situation and the wind speed. All the other predictors show a significance.
So we have a another look at it without the weather situation and wind speed.

```{r, echo=FALSE, eval=FALSE}
glm.binomial.hour.3 <- glm(ilogit(cnt) ~ season + mnth + hr + weekday + workingday + temp + hum + weathersit:temp + weathersit:hum, data = hour, family = "quasibinomial")

summary(glm.binomial.hour.3)
```
...
```{r, echo=FALSE}
glm.binomial.1 <- glm(ilogit(cnt) ~ season + yr + mnth + hr + holiday + weekday + workingday + weathersit + temp + hum + windspeed, data=hour, family="quasibinomial")
...
par(oma=c(0, 0, 0, 5))
plot(as.Date(hour.test_in$dteday),predicted.binomial.test.1, type="l",col="firebrick3",  xlab = " ", ylab ="ilogit of Number of Bikes used")
...
For this we use the percentage of registered users out of the total number of users as the response variable.
```{r, echo=TRUE, results='hide'}
glm.binomial.2 <- glm((registered/cnt) ~ season + yr + mnth + hr + holiday + weekday + workingday + weathersit + temp + hum + windspeed, data=hour, family="quasibinomial")
...
```{r, echo=FALSE, out.width = "60%"}
par(oma=c(0, 0, 0, 5))
plot(as.Date(hour.test_in$dteday),predicted.binomial.test.2,type="l",
     col="firebrick3",  xlab = " ", ylab ="Share of Registered Users")
lines(as.Date(hour.test_in$dteday),(hour.test_in$registered/hour.test_truth),
      pch=20,col="deepskyblue2", type="p", cex = .3)
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
       c("Prediction", "True Values"),col=c("firebrick3", "deepskyblue2"),
       pch=c(1, 2), lty=c(1,2))
```
...
```{r, echo=FALSE, results='hide'}
glm.binomial.3 <- glm((registered/cnt) ~ season + yr + mnth + holiday + weekday + workingday +  weathersit+ temp + hum + windspeed, data=day, family="quasibinomial")
...
```{r, echo=FALSE, out.width = "60%"}
par(oma=c(0, 0, 0, 5))
plot(as.Date(day.test_in$dteday),predicted.binomial.test.3,type="l",col="firebrick3",  xlab = " ", ylab ="Share of Registered Users")
lines(as.Date(day.test_in$dteday),(day.test_in$registered/day.test_truth),type="l",col="deepskyblue2")
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
...
par(oma=c(0, 0, 0, 5))
plot(as.Date(day.test_in$dteday),predicted.binomial.test.4,type="l",col="firebrick3",  xlab = " ", ylab ="Ratio of Registered/Total Users")
lines(as.Date(day.test_in$dteday),(day.test_in$registered/day.test_truth),type="l",col="deepskyblue2")
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
       c("Prediction", "True Values"),col=c("firebrick3", "deepskyblue2"), pch=c(1, 2), lty=c(1,2))
```
...
```{r, echo=TRUE, results='hide'}
glm.binomial.5 <- glm((registered/cnt) ~ yr + mnth + holiday + weekday + workingday, data=day, family="quasibinomial")
...
```{r, echo=FALSE}
par(oma=c(0, 0, 0, 5))
plot(as.Date(day.test_in$dteday),(predicted.binomial.test.5),type="l",col="firebrick3",  xlab = " ", ylab ="Ratio of Registered/Total Users")
lines(as.Date(day.test_in$dteday),(day.test_in$registered/day.test_truth),type="l",col="deepskyblue2")
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
       c("Prediction", "True Values"),col=c("firebrick3", "deepskyblue2"), pch=c(1, 2), lty=c(1,2))
```
...
```{r, echo=FALSE}
glm.pois.hour <- glm(cnt ~ season + yr + mnth + hr+ holiday + weekday + workingday + weathersit + temp + atemp + hum + windspeed, data = hour,
family = "quasipoisson")
...
With the temperature and the assumed temperature being very similar, we drop the assumed temperature from the model, with the measured temperature being more tangible. we also drop the weather situation as we still keep all the other weather variables. Furthermore, we drop the dates.


```{r, echo=TRUE, results='hide'}
glm.pois2.hour <- glm(cnt ~ season + yr + mnth + hr+ holiday + weekday + workingday + temp + hum + windspeed, data = hour, family = "quasipoisson")
...
```{r, echo=FALSE}
glm.poisson.1 <- glm(cnt ~ season + yr + mnth + hr + holiday + weekday + workingday + weathersit + temp + hum + windspeed, data=hour, family=quasipoisson)
...
```{r, echo=FALSE , out.width= "60%"}
par(oma=c(0, 0, 0, 5))
plot(as.Date(hour.test_in$dteday),predicted.poisson.test.1,type="l",
     col="firebrick3",  xlab = "  ", ylab ="Number of Bikes used")
lines(as.Date(hour.test_in$dteday),hour.test_truth,type="p",col="deepskyblue2", 
      cex = .3, pch=20)
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
       c("Prediction", "True Values"),col=c("firebrick3", "deepskyblue2"),
       pch=c(1, 2), lty=c(1,2))
...
As hourly numbers vary strongly, we get quite some variance. 
Now we want to see if this model would fit better on a daily basis.

```{r, echo=FALSE}
glm.poisson.2 <- glm(cnt ~ season + yr + mnth + holiday + weekday + workingday + weathersit + temp + hum + windspeed, data=day, family=quasipoisson)
...
```{r, echo=FALSE}
par(oma=c(0, 0, 0, 5))
plot(as.Date(day.test_in$dteday),predicted.poisson.test.2,type="l",col="firebrick3", xlab = "  ", ylab ="Number of Bikes used") 
lines(as.Date(day.test_in$dteday),day.test_truth,type="l",col="deepskyblue2")
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
       c("Prediction", "True Values"),col=c("firebrick3", "deepskyblue2"), pch=c(1, 2), lty=c(1,2))
```
...
```{r, echo=TRUE, results='hide'}
glm.poisson.3 <- glm(cnt ~ season + yr + mnth + holiday + weekday + workingday + weathersit + temp, data=day, family=quasipoisson)
...
```{r, echo=FALSE}
summary(predicted.poisson.test.3)
```
...
```{r, echo=FALSE, eval=FALSE}
par(oma=c(0, 0, 0, 5))
plot(as.Date(day.test_in$dteday),predicted.poisson.test.3,type="l",col="firebrick3", xlab = "  ", ylab ="Number of Bikes used") 
lines(as.Date(day.test_in$dteday),day.test_truth,type="l",col="deepskyblue2")
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
       c("Prediction", "True Values"),col=c("firebrick3", "deepskyblue2"), pch=c(1, 2), lty=c(1,2))
```

### Conclusion: GLM Poisson 
...
```
The svm.poly.2 achieved a RMSE of 105.9098 with cost 1, degree 1 and coef 1. For the model testing, the svm.poly.2 will be used. Let's see how well the model works with the test data.
...
```
The svmRadial kernel uses the kernlab package. The radial kernel performed so far best, with a RSME of  51.06311. The final values used for the model were sigma = 0.0107185 and C = 1.
...
set.seed(9876)
#linear model
print(paste("SVM linear kernel", rmse(dt.test$cnt, pred.linear)))

#polynomial model
print(paste("SVM polynomial kernel", rmse(dt.test$cnt, pred.poly)))

#radial model
print(paste("SVM radial kernel", rmse(dt.test$cnt, pred.radial)))
```
The radial kernel clearly outperformed the other two. the radial kernel has a RSME of 45.8, the polynomial an RSME of 107.2 and the linear kernel a RSME of 106.2.
Therefor the winner of the support vector machine models is the one with the radial kernel. It also takes the most time to run. The linear model is much faster but has a higher RMSE value.
...
Here we show an example of how this is done. The list is not conclusive.
...
## ANN Model 1:  Computing a Range of Model
...
```{r, echo=FALSE, out.width="60%"}
plot(control_1, pred_1, col='orange', cex = .3, pch=20,
     ylab = "predicted count NN", xlab = "real count")
abline(0,1)
```

## ANN Model 2: Layer 4/0/0, Threshold 0.01
...
### Plot Prediction against Real Data
```{r, echo=FALSE, out.width="60%"}
plot(control_2, pred_2, col='orange', cex = .3, pch=20,
     ylab = "predicted count NN", xlab = "real count")
abline(0,1)
```
...
model_4 = neuralnet(cnt ~ hum + temp + windspeed + weathersit_1 + weathersit_2 +
                      ...
                      hr_7 + hr_8 + hr_9 + hr_10 + hr_11 + hr_12 + hr_13 +
                      hr_14 + hr_15 + hr_16 + hr_17 + hr_18 + hr_19 + hr_20 +
                      hr_21 + hr_22 + hr_23 + weekday_1 + weekday_2 +
                      weekday_3 + weekday_4 + weekday_5 + weekday_6 +
                      weekday_7 + mnth_1 + mnth_2 + mnth_3 + mnth_4 + mnth_5 +
                      mnth_6 + mnth_7 + mnth_8 + mnth_9 + mnth_10 + mnth_11 +
                      mnth_12, data = train_2,
                    hidden = c(4),linear.output = TRUE, threshold = 0.05,
                    stepmax = 600000, lifesign.step = 1000, lifesign = "full")
```
...
```{r, echo=FALSE, out.width="60%"}
plot(control_4, pred_4, col='orange', cex = .3, pch=20, ylab = "predicted count NN", xlab = "real count")
abline(0,1)
```

## ANN Models 5: Computing a Range of Model
...
### Plot Prediction against Real Data
```{r, echo=FALSE, out.width="60%"}
plot(control_6, pred_6, col='orange', cex = .3, pch=20, ylab = "predicted counts NN", xlab = "real counts")
abline(0,1)
```

## ANN Model 9: more Predictors, Layer 7/3/0, Threshold 0.01
...
### Plot Prediction against Real Data
```{r, echo=FALSE, out.width="60%"}
plot(control_9, pred_9, col='orange', cex = .3, pch=20, ylab = "predicted counts NN", xlab = "real counts")
abline(0,1)
```

## ANN Model 11: all Predictors, Layer 7/3/0, Threshold 0,01

It turned out that the year is an important predictor. Now, to make sure that no information is missed, all meaningful predictors are used. The same model layout of model 9 is taken. The Result is an RSME of 46.
...
## ANN Models 10: Computing a Range of Model
...
To buy a new bike will cost us 300$.
The bikes we own cost us on average 1$ a day for maintenance and so on, regardless whether they are in use or not.
If a bike is in use, we will earn 3$ a day.

The demand depends on the months. The daily demand coefficients look as follows:

      Jan   Feb   Mar   Apr   May   Jun   Jul   Aug   Sep   Oct   Nov   Dec
a:    1.5   1.5   1.9   2.3   2.7   3.0   3.2   3.5   3.5   3.3   3.0   2.2
      a<sub>1    a<sub>2 ...                                            a<sub>12   
...
$$
x * 300 = 2 * (3 * \sum_{i = 1}^{12}{(b_i * min(x + 2000, a_i * 1000) - 1 * 365 * (x + 2000))})
$$
...
print(paste("RMSE Linear Model: ",RMSE_Linear_Model <- sqrt(mean((exp(control_log) - exp(predict_3))^2))))

#RMSE Gam Model (RMSE of 90.4)
print(paste("RMSE GAM Model: ",RMSE_gam_5 <- sqrt(mean((exp(control_log) - exp(predict_gam_5))^2))))

#RMSE GLM 
# Binomial (RMSE of 0.06111774) 
print(paste("RMSE Binomial Model: ",RMSE_glm_binomial_3 <- rmse((day.test_in$registered/day.test_truth), predicted.binomial.test.3)))
# Poisson (RMSE of 134 for hourly data)
print(paste("RMSE Poisson Model: ",RMSE_glm_poisson_1 <- rmse(day.test_truth, predicted.poisson.test.1)))


#RMSE SVM Model wit a radial kernel (RSME o 47.66508)
print(paste("RMSE SVM Model", rmse(dt.test$cnt, pred.radial)))
                                   
#RMSE NN Model
print(paste("RMSE NN Model",45))
```
...
We got best prediction performance with the black-box models. The weightings of the predictors are more transparent to the bike rental company. Thus it depends on whether the company needs to know which dependent variables influence the number of rental the most, or not. Further, if the company wants to use the model to predict bike rentals, it must accept some inaccuracy and take this into account for scheduling staff or other factors.
If we assume an RSME of 47 for the Support Vector Machine, this means a percentage error of about 25% based on the average of 189 bikes per hour. This error should be added to the predicted numbers.
