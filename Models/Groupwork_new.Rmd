...
First we have a look at how the different variables are corresponding with number of bicycles, that are being used daily: 
```{r, echo=FALSE}
pairs.daily <- pairs(cnt ~ season + yr + mnth + holiday + weekday + workingday + weathersit + temp + atemp + hum + windspeed + casual + registered, data = day, upper.panel = panel.smooth)
```
And with the number of bicycles, that are being used hourly:
```{r, echo=FALSE}
pairs.hour <- pairs(cnt ~ season + yr + mnth + hr+ holiday + weekday + workingday + weathersit + temp + atemp + hum + windspeed + casual + registered, data = hour, upper.panel = panel.smooth)
```


What we can see here, is that it looks like there is an uptake in bicycle usage when the temperature is rising (up to a certain point). When it gets too hot, the demand seems to lower again. This also corresponds in the usage during the different seasons, with the least demand in winter.
...
```
Here we can see that spring (season 1) is the least popular season for bike usage. This seems a little contraintuitiv as one would assume it would be winter. So we have a closer look at the usage per month.

```{r, echo=FALSE}
ggplot(day,aes(x=(mnth= month.abb[mnth]), y=cnt, fill=season)) + 
  geom_boxplot()+labs(x="Months", y="Number of Bikes Used per Day") + scale_x_discrete(limits = month.abb)
...
```{r, echo=TRUE}
glm.binomial.hour.1 <- glm(ilogit(cnt) ~ season + yr + mnth + hr+ holiday + weekday + workingday + weathersit + temp + atemp + hum + windspeed, data = hour, family = "quasibinomial")
...
```{r, echo=TRUE}
par(oma=c(0, 0, 0, 5))
plot(as.Date(hour.test_in$dteday),predicted.binomial.test.2,type="l",col="firebrick3",  xlab = " ", ylab ="Share of Registered Users")
lines(as.Date(hour.test_in$dteday),(hour.test_in$registered/hour.test_truth),type="p",col="deepskyblue2")
legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,
...
```
Now let's look at it one more time, without the season (to concentrate on the months) and without the working day variable.

```{r, echo=FALSE}
glm.pois3.hour <- glm(cnt ~ yr+ mnth + hr + holiday + weekday +  temp + hum + windspeed, data = hour.train, family = "quasipoisson")
...
```{r, echo=FALSE}
par(oma=c(0, 0, 0, 5))
plot(as.Date(hour.test_in$dteday),predicted.poisson.test.1,type="l",col="firebrick3",  xlab = "  ", ylab ="Number of Bikes used")
...
```
Interestingly, in this plot, the number of total rentals for spring is lower then for winter. But also the temperature when the rentals happen has more variablity than during the spring. The Temperature Celsius is normalized, therefore it ranges between 0 and 1.
...
rmse(dt.test$cnt, pred.radial)
```
The radial kernel clearly outperformed the other two. the radial kernel has a RSME of 49.55235, the polynomial an RSME of 108.1839 and the linear kernel a RSME of 106.9027.
...
# Poisson
RMSE_glm_poisson_2 <- rmse(day.test_truth, predicted.poisson.test.2)
