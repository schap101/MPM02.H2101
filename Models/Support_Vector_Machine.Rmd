...
# Decision boundary
Code to visualize the decision boundary for support vector machines
```{r}
decision_boundary <- function(model, data, formula, 
                            proba = FALSE, 
                            xlab = NULL, 
                            ylab = NULL) {
  ...
  if (proba == FALSE) {
    ...
    if (clss == "knn3") {
      ...
      result <- predict(model, zz, type = "prob")
    } else if (clss == "naiveBayes") { 
      result <- predict(model, zz, type = "class")
    } else if (clss == "naive_bayes") { 
      result <- predict(model, zz, type = "class")
    } else {
    ...
    if (clss == "knn3") {
      result <- predict(model, zz)
    } else if (clss == "randomForest.formula") { 
      result <- predict(model, zz, type = "prob")
    } else if (clss == "rpart") { 
      result <- predict(model, zz, type = "prob")
    } else if (clss == "train") { 
      result <- predict(model, zz, type = "prob")
    } else if (clss == "svm.formula") { 
