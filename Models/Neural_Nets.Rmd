...
```{r}
set.seed(123)
indices <- createDataPartition(df$cnt, p=.2, list = F)
...
```{r}
set.seed(44)
tuGrid_2 <- expand.grid(.layer1=c(3:6), .layer2=c(0:4), .layer3=c(0:3))
...
```{r}
set.seed(42)
final_model = neuralnet(cnt ~ hr + weekday + mnth, train_2, hidden = 3 , li
plot(final_model)
```
