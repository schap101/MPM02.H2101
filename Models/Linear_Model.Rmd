...
We evaluate the accuracy of both models by testing them with the test data partition. The R-squared will serve as an indication of the accuracy.
...
The predictors used in the model lm.fit.4.train explain about 80 % of the variance in cnt (number of bike rentals) according to the R-squared values. It would have been ideal the look at the RSE values to estimate the percentage error of the number of bike rentals. However since the cnt variable has been log transformed we where not able to interpret these the RSE appropriate.
...
### Are there Potential Problem Fitting this data with a Linear Model 
